...
#include "exoskeleton_safety_msgs/srv/set_mode.hpp"
...
protected:
  ...
  {
    node_ = node;
    client_ = node_->create_client<exoskeleton_safety_msgs::srv::SetMode>(
      "/bridge/set_mode");
  }
  ...
  void request_mode(const std::string & mode)
  {
    ...
    auto req = std::make_shared<exoskeleton_safety_msgs::srv::SetMode::Request>();
    req->mode = mode;

    auto response_cb = [this, mode](
      rclcpp::Client<exoskeleton_safety_msgs::srv::SetMode>::SharedFuture future)
    {
      try {
  ...
  rclcpp::Client<exoskeleton_safety_msgs::srv::SetMode>::SharedPtr client_;
